...
class DetailProduct extends React.Component {
      ...
      await fetch_url_get(
        ...
        if (data.declinaison.length == 0) {
          ...
          await data.declinaison.forEach(async (item) => {
            ...
            await this.changeDeclinaison(opt_value, id, qt, id_group_customer);
            // console.log(item)
          });
  ...
  updateFinalPrice = async () => {
    ...
    let id_default_combination_data =
          ...
          this.props.route.params.price ? (
            ...
            >
              {this.props.route.params.price
                ...
                : parseFloat(this.state.product.product.product.price).toFixed(
                    2
                  )}
              € TTC
            </Text>
  ...
  render() {
    ...
    if (
      ...
      var combinationTplNew = this.state.product.declinaison.map(
        (combinationVar, i) => {
          var combinationId = combinationVar.id;
    ...
    if (
      ...
      var ImgUrl =
        api_url +
        productInfo.id_default_image +
        "-medium_default/" +
        productInfo.link_rewrite.language[0] +
        ".jpg";
      var condition = productInfo.condition;
      if (
        productInfo.description_short.language &&
        typeof productInfo.description_short.language != "object" &&
        !Array.isArray(productInfo.description_short.language) &&
        productInfo.description_short.language !== null
      ) {
      ...
      if (
        ...
        typeof productInfo.description.language != "object" &&
        !Array.isArray(productInfo.description.language) &&
        productInfo.description.language !== null
      ) {
                ...
                <View style={{ flexDirection: "row" }}>
                  <Text style={detail_product_styles.price}>
                    {this.state.price_final
                      ...
                      ? parseFloat(this.props.route.params.price).toFixed(2)
                      : parseFloat(
                          this.state.product.product.product.price
                        ).toFixed(2)}
                    € TTC
                  </Text>
                  {combinationTplNew}
